...
import { publishSyncState, subscribeToSync } from './redisSync';
import {
  type Mark,
...
export function startWebSocketServer(port: number, serverId: string, redisSync: RedisSync): void {
  ...
  wss.on('connection', (socket: ExtendedWebSocket) => {
    ...
    socket.on('close', async () => {
      ...
      try {
        await redisSync.pub.del(lockKey);
      } catch (err) {
        console.error('Error releasing Redis lock on close:', err);
      }
    });
  });
}
...
async function handleJoinMessage(
  ...
  const lockKey = `game:${gameId}:player:${requestedMark}`;

  try {
    const result = await redisSync.pub.set(lockKey, serverId, { NX: true });
    if (result === null) {
      sendError(socket, `Mark ${requestedMark} is already taken`);
      return;
    }
  } catch (err) {
    console.error('Error acquiring Redis lock:', err);
    sendError(socket, 'Internal error (Redis)');
    return;
  }
